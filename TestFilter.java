...
public class TestFilter extends DocumentFilter {
	...
	public void replace(FilterBypass fb, int offset, int length, String str, AttributeSet attr)
		...
		/*
		 * if(offset % 2 == 0) { StyleConstants.setBold(attrs, true);
		 * StyleConstants.setForeground(attrs, Color.red); super.replace(fb,
		 * offset, length, str, attrs); } else { StyleConstants.setBold(attrs,
		 * false); StyleConstants.setForeground(attrs, Color.black);
		 * super.replace(fb, offset, length, str, attrs); }
		 */
		/*
		 * int ind = EditorArea.getEditor().indexOf(keyWord); if(ind != 0){
		 * StyleConstants.setBold(bold, true); super.replace(fb, ind,
		 * keyWord.length(), keyWord, bold); }
		 */
	}
	...
	public String getContext(AttributeSet set) {
		if (set.isDefined(charState)) {
			return (String) (set.getAttribute(charState));
		}
		return "";
	}
	...
	/**
	 * Inserts each "piece" of the 2d array individually, making a new element
	 * each time
	 * 
	 * @param pieces
	 *            A 2d string array in the format returned by parseInsertion
	 */
	public void replaceAll(FilterBypass fb, int offset, int length, String[][] pieces, AttributeSet set)
			throws BadLocationException {
		for (String[] piece : pieces) {
			SimpleAttributeSet named = new SimpleAttributeSet(set);
			// System.out.println("Name of \"" + piece[1] + "\" is " +
			// set.getAttribute(AbstractDocument.ElementNameAttribute));
	...
	/**
	 * based on the context, returns and modifies an attributeset to have syntax
	 * highlighting.
	 */
	...
	/**
	 * Assigns contexts to the different structures within the string. Also goes
	 * through the string and breaks it up into syntax structures.
	 * 
	 * @param context
	 *            the context that the insertion was inserted into
	 * @param insertion
	 *            the string to parse
	 * @return a 2d string array where each String[] is of length two. the first
	 *         string is the context, and the second string is the thing that
	 *         context is describing. Example:<br>
	...
	public String[][] parseInsertion(String context, String insertion) {
		...
		while (i < insertion.length()) {
			...
			// System.out.print(" Determined '" + current + "' from '" +
			// insertion.charAt(i) + "' in '" + previous + "'");
			if (previous.equals(current)) {
				// System.out.println("; Continuing \"" + building + "\" in '" +
				// current + "' with '" + insertion.charAt(i));
	...
	/**
	 * determines the context that this character should have based on the
	 * context it was inserted into
	 */
	public String determineState(String previous, char c) {
		...
		if (previous.equals("tag-special")) {
			if (c == '-') {
				return "tag-special-comment-partial";
			}
			return "tag-name";
		}
		...
		if (previous.equals("html-comment")) {
			if (c == '-') {
				return "html-comment-1";
			}
			return "html-comment";
		}
		if (previous.equals("html-comment-1")) {
			if (c == '-') {
				return "html-comment-2";
			}
			return "html-comment";
		}
